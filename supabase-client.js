...
window.addEventListener('DOMContentLoaded',()=>{
  // Logo local incorporada ao projeto para não depender do CDN temporário do Instagram.
  ...
  function tone(freq,when,duration,type='sine',gain=.03){
    ...
    o.type=type;o.frequency.setValueAtTime(freq,when);f.type='lowpass';f.frequency.value=900;
    g.gain.setValueAtTime(.0001,when);g.gain.exponentialRampToValueAtTime(gain,when+.8);g.gain.exponentialRampToValueAtTime(.0001,when+duration);
  ...
  function playBar(){
    ...
    chord.forEach((f,i)=>tone(f,now,5.6,'sine',i===0?.018:.011));tone(chord[0]/2,now,2.5,'triangle',.022);
    const notes=[chord[1]*2,chord[2]*1.5,chord[1]*1.5];tone(notes[step%notes.length],now+1.2,1.7,'sine',.009);tone(notes[(step+1)%notes.length],now+3.1,1.5,'sine',.007);step++;
  }
